<?php
	...
	class Authorization {
		...
		function registration( $add ) {//Добавить запись в sp_user_balance
			...
			if(count($checkLogin)==0 && count($checkEmail)==0) {
				...
				$user = Database::getrow('birzha_users', '*', 'login', $reg['login'], 0);
				$balance['user_id'] = $user['id'];
				$balance['user_balance'] = 0;
				$balance['curency_id'] = 1;
				Database::insert('sp_users_balance', $balance);
				
				return $return;
			
			} else {
				$return['message'] = 'Такой логин или email уже заняты';
				$return['status'] = 0;
				return $return;
			}
		...
		function remember( $add ) {
			...
			if (count($user)==0) {
				...
				<p><a href="http://sp-kubani.inmtoo.com/index.php/repass/tokenverification/?tokenkey='.$token['token'].'">http://myzapros.ru/index.php/repass/tokenverification/?tokenkey='.$token['token'].'</a></p>
				';
	...
	/*	function change_password_from_old($add) {
			$users = Database::getrows($table = 'birzha_users', $fields = '*', $parametr = 'id', $value = $add['id']);
			$user = $users[0];
			//$salt = $user['salt'];
			$unsaltedpswrd = md5(md5($user['salt']).md5($add['password']));
